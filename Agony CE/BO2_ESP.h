...
void _Radar()
{
	...
	for (int i = 0; i < 1024; i++)
	{
		...
		if (Type == ET_PLAYER) {
			...
			Shader = Material_RegisterHandle("hud_grenadepointer", 0);
		...
		if (Type == ET_MISSLE)
			...
			CG_DrawRotatedPicPhysical(0x82CBC168, FinalX - 10, FinalY - 10, 20, 20, (i == cgArray_s->ClientNumber || Type == ET_ITEM) ? 0 : (cgArray_s->ViewAngle.y - cgArray_s->ClientInfo[i].viewAngle.y), color, Shader);
	}
}
...
void _Compass()
{
	...
	for (int i = 0; i < 1024; i++)
	{
		...
		if (Shader)
			CG_DrawRotatedPicPhysical(0x82CBC168, CircleX - dwWidth / 2, CircleY - dwHeight / 2, dwWidth, dwHeight, FinalA, color, Shader);
	}
}
...
void _ESPClient(int client, float sColor[4])
{
		...
		WorldToScreen(HeadOrigin, &HeadLocation.x, &HeadLocation.y))
	{
		...
		float fontSize = 0.45;
		char *fontName = "fonts/720/normalFont";
		...
		if (bDrawWeapon) {
			...
			if (Shader) {
				CG_DrawRotatedPicPhysical(0x82CBC168, FeetLocation.x - 30, FeetLocation.y, 60, 30, 0, colWHITE, Shader);
			}
		}
		...
		int clientHealth = *(int*)(0x833D0640 + 0x1A8 + (client * 0x31C));

		int healthSize = clientHealth * (fHeight / 100);

		float healthColor[4] = { (float)1 - (clientHealth / 100) , (float)clientHealth / 100, 0,  1 };

		DrawShader(FeetLocation.x - fWidth / 2 - 5, FeetLocation.y - fHeight - 1, 3, fHeight + 3, colSHADER, "white");
		DrawShader(FeetLocation.x - fWidth / 2 - 4, FeetLocation.y - fHeight + (fHeight - healthSize), 1, healthSize - 2, healthColor, "white");
	}
}
			...
			if (WorldToScreen(Centity_s[i].Origin, &Location.x, &Location.y))
			{
				if (Centity_s[i].Type == ET_ITEM)
				{
					if ((Centity_s[i].State & (1 << 6)) != 0) {
						...
						int dwTempShader = GetWeaponShader(WeaponID);
						if (dwTempShader)
							CG_DrawRotatedPicPhysical(0x82CBC168, Location.x - 20, Location.y, 40, 20, 0, colWHITE, dwTempShader);
