void _DrawMenu()
{
	DrawText(szMenuName, 50, 700, "fonts/HudBigFont", 0.5, defaultColor);
	...
	if (MenuVars.openState == 1)
	{
		...
		for (int i = bSubToggle ? 1 : 0; i < MenuOption; i++)
		{
			...
			if (MenuVars.Option_Toggle[CurrentMenu][i] != NULL)
			{
				...
				if (MenuVars.bComboBox[CurrentMenu][i]) {
					...
					DrawText(ArrayText, t_flX + 14 + 40 - ((TextWidth(ArrayText, szOptionFont) * 0.34) / 2), t_flY, szOptionFont, 0.34, defaultText);
			}
				...
				DrawText(ArrayText, t_flX + 40 - ((TextWidth(ArrayText, szOptionFont) * 0.34) / 2), t_flY + 6, szOptionFont, 0.34, defaultText);
				t_flY += 5;
		...
		if (MenuVars.bComboBox[CurrentMenu][CurrentOption]) {
			...
			for (int i = 0; i < ComboSize; i++) {
				DrawText(ComboArray[i], flComboX + 40 - ((TextWidth(ComboArray[i], "fonts/HudBigFont") * 0.4) / 2), flComboY + 3 + (15 * i), "fonts/HudBigFont", 0.4, defaultText);
...
void Menu_PaintAll(int r3) {
	...
	DrawText(_resolution, 50, uicontext.screenHeight - 170, "fonts/HudBigFont", 0.5, defaultText);
...
void CG_DrawNightVisionOverlay() {
	GetStructs();
	...
	if (isEntityPlayer(cgArray_s->ClientNumber)) {
		...
		if (bShowHost) {
			...
			_snprintf(_host, 64, "Host: %s", pCgs->hostname);
			_snprintf(_map, 64, "Map: %s", pCgs->mapname);
			_snprintf(_game, 64, "Game: %s", pCgs->gamemode);

			DrawText(_host, 50, cgArray_s->RefDef.Height - 230, "fonts/HudBigFont", 0.5, defaultText);
			DrawText(_map, 50, cgArray_s->RefDef.Height - 210, "fonts/HudBigFont", 0.5, defaultText);
			DrawText(_game, 50, cgArray_s->RefDef.Height - 190, "fonts/HudBigFont", 0.5, defaultText);

			if (MenuVars.openState == 0) {
				DrawText("Target Details", cgArray_s->RefDef.Width - TextWidth("Target Details", "fonts/HudBigFont") * 0.5 - 50, 250, "fonts/HudBigFont", 0.5, defaultColor);
				...
				if (ClosestPlayer != -1) {
					...
					DrawText(_id, cgArray_s->RefDef.Width - TextWidth(_id, "fonts/HudBigFont") * 0.5 - 50, 270, "fonts/HudBigFont", 0.5, defaultText);
					DrawText(_name, cgArray_s->RefDef.Width - TextWidth(_name, "fonts/HudBigFont") * 0.5 - 50, 290, "fonts/HudBigFont", 0.5, defaultText);
					DrawText(_distance, cgArray_s->RefDef.Width - TextWidth(_distance, "fonts/HudBigFont") * 0.5 - 50, 310, "fonts/HudBigFont", 0.5, defaultText);
					DrawText(_visible, cgArray_s->RefDef.Width - TextWidth(_visible, "fonts/HudBigFont") * 0.5 - 50, 330, "fonts/HudBigFont", 0.5, defaultText);

				}
				else {
					DrawText("N/A", cgArray_s->RefDef.Width - TextWidth("N/A", "fonts/HudBigFont") * 0.5 - 50, 270, "fonts/HudBigFont", 0.5, defaultText);
				}
			}
		}
...
{
	if (!Dvar_GetBool("cl_ingame") || !*(int*)0x823F28A0 || !*(int*)0x823F5054 || !*(int*)0x82435AB8)
...
int R_DrawXModelSkinnedCached(int pDevice, int Args) {
	...
	if (Device && bChams) {
		...
		if (strstr(pShader, "_op_force_") || strstr(pShader, "viewmodel_guard")) { // Red
			drawPlayer = 1;
		...
		if (strstr(pShader, "_marine_")) { // Green
			drawPlayer = 2;
		...
		if (strstr(pShader, "_ghillie_")) {
			drawPlayer = 1;

			if (bWireframe) {
				Device->SetRenderState(D3DRS_FILLMODE, D3DFILL_WIREFRAME);

				Device->SetTexture(0, BlackTexture);
		...
		if (strstr(pShader, "_arab_") || strstr(pShader, "_opforce_")) {
			drawPlayer = 1;
		...
		if (strstr(pShader, "_usmc_") || strstr(pShader, "_sas_")) {
			drawPlayer = 2;
