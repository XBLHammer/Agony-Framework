...
void _Radar()
{
	...
	{
		int Type;
		if (isEntityPlayer(i)) Type = ET_PLAYER;
		...
		if (Shader)
			CG_DrawRotatedPicPhysical(0x82A06718, FinalX - 10, FinalY - 10, 20, 20, (i == cgArray_s->ClientNumber || Type == ET_ITEM) ? 0 : (cgArray_s->ViewAngle.y - cgArray_s->ClientInfo[i].viewAngle.y), color, Shader);
	}
}
...
void _Compass()
{
	...
	for (int i = 0; i < 18; i++)
	{
		...
		if (Shader)
			CG_DrawRotatedPicPhysical(0x82A06718, CircleX - dwWidth / 2, CircleY - dwHeight / 2, dwWidth, dwHeight, FinalA, color, Shader);
	}
}
...
void _ESPClient(int client, float sColor[4])
{
		...
		WorldToScreen(HeadOrigin, &HeadLocation.x, &HeadLocation.y))
	{
		...
		if (bDrawWeapon) {
			...
			if (Shader) {
				CG_DrawRotatedPicPhysical(0x82A06718, FeetLocation.x - 30, FeetLocation.y, 60, 30, 0, colWHITE, Shader);
			}
		}
