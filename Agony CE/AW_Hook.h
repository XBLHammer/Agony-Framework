void _DrawMenu()
{
	DrawText(szMenuName, 50, cgArray_s->RefDef.Height - 250, "fonts/smallDevFont", 1, colRED);
	...
	if (MenuVars.openState == 1)
	{
		...
		DrawText(_szName, flMenuX + (flMenuW / 2) - ((TextWidth(_szName, szHeaderFont) / 2) * flHeaderSize), flMenuY + 4 + TextHeight(szHeaderFont) * flHeaderSize, szHeaderFont, flHeaderSize, defaultText);
		...
		for (int i = bSubToggle ? 1 : 0; i < MenuOption; i++)
		{
			...
			if (MenuVars.Option_Toggle[CurrentMenu][i] != NULL)
			{
				...
				if (MenuVars.bComboBox[CurrentMenu][i]) {
					...
					DrawText(ArrayText, t_flX + 14 + 40 - ((TextWidth(ArrayText, szOptionFont) * 0.6) / 2), t_flY, szOptionFont, 0.6, defaultText);
			}
				...
				DrawText(ArrayText, t_flX + 40 - ((TextWidth(ArrayText, szOptionFont) * 0.6) / 2), t_flY + 6, szOptionFont, 0.6, defaultText);
				t_flY += 5;
		...
		if (MenuVars.bComboBox[CurrentMenu][CurrentOption]) {
			...
			for (int i = 0; i < ComboSize; i++) {
				DrawText(ComboArray[i], flComboX + 40 - ((TextWidth(ComboArray[i], szOptionFont) * 0.6) / 2), flComboY + 3 + (15 * i), szOptionFont, 0.6, defaultText);
		...
		/*if (CurrentMenu == 6) {
			if (isEntityPlayer(CurrentOption)) {
...
void H_R_SetFrameFog(int r3)
{
	if (r3 != 0xFFFFFFFF82ACB938) return;

	if (!*(int*)0x82AD0298 || !*(int*)0x82B0D308 || !*(int*)0x82ACFE30) return;
	...
	{
		float xHairSize = (8 + (*(float*)((*(int*)(0x82AD0298)) + 0x5B8) / 4));
	...
	if (bShowHost) {
		if (MenuVars.openState == 0) {
			DrawText("Target Details", cgArray_s->RefDef.Width - TextWidth("Target Details", "fonts/smallDevFont") - 50, 250, "fonts/smallDevFont", 1, defaultColor);
			...
			if (ClosestPlayer != -1) {
				...
				char *szName = cgArray_s->ClientInfo[ClosestPlayer].Name;

				_snprintf(_id, 64, "ID: %i", ClosestPlayer);
				_snprintf(_name, 64, "Name: %s", szName);
				...
				DrawText(_id, cgArray_s->RefDef.Width - TextWidth(_id, "fonts/smallDevFont") - 50, 270, "fonts/smallDevFont", 1, defaultText);
				DrawText(_name, cgArray_s->RefDef.Width - TextWidth(_name, "fonts/smallDevFont") - 50, 290, "fonts/smallDevFont", 1, defaultText);
				DrawText(_distance, cgArray_s->RefDef.Width - TextWidth(_distance, "fonts/smallDevFont") - 50, 310, "fonts/smallDevFont", 1, defaultText);
				DrawText(_visible, cgArray_s->RefDef.Width - TextWidth(_visible, "fonts/smallDevFont") - 50, 330, "fonts/smallDevFont", 1, defaultText);
			}
			else {
				DrawText("N/A", cgArray_s->RefDef.Width - TextWidth("N/A", "fonts/smallDevFont") - 50, 270, "fonts/smallDevFont", 1, defaultText);
			}
		}
	}
...
{
	if (!*(int*)0x82AD0298 || !*(int*)0x82B0D308 || !*(int*)0x82ACFE30) { return H_CL_CreateNewCommandsStub(r3); }
