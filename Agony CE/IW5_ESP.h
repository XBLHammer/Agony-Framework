	...
	for (int i = 0; i < 2048; i++)
	{
		...
		if (Type == ET_ITEM) {
			...
			if (strcmp(GetRawWeaponName(WeaponID), "scavenger_bag_mp")) {
				...
				Shader = Material_RegisterHandle("specialty_scavenger", 0);
			}
		}
		...
		if (Type == ET_MISSLE)
			...
			CG_DrawRotatedPicPhysical(0x827322C0, FinalX - 10, FinalY - 10, 20, 20, (i == cgArray_s->ClientNumber || Type == ET_ITEM) ? 0 : (cgArray_s->ViewAngle.y - Centity_s[i].viewAngle.y), color, Shader);
	}
}
...
void _Compass()
{
	...
	for (int i = 0; i < 2048; i++)
	{
		...
		if (Shader)
			CG_DrawRotatedPicPhysical(0x827322C0, CircleX - dwWidth / 2, CircleY - dwHeight / 2, dwWidth, dwHeight, FinalA, color, Shader);
	}
}
...
void _ESPClient(int client, float sColor[4])
{
		...
		WorldToScreen(HeadOrigin, &HeadLocation.x, &HeadLocation.y))
	{
		...
		if (bDrawWeapon) {
			...
			SetTextCenteredWithBackGround(szWeapon, fontName, FeetLocation.x, FeetLocation.y + 17, fontSize, fontSize, colWHITE, colSHADER, false);
		}
				...
				if (Centity_s[i].Type == ET_ITEM)
				{
					if (Centity_s[i].Alive & 1) {
						...
						if (strcmp(GetRawWeaponName(WeaponID), "scavenger_bag_mp")) {
							...
							int dwTempShader = GetWeaponShader(WeaponID);
							if (dwTempShader)
								CG_DrawRotatedPicPhysical(0x827322C0, Location.x - 20, Location.y, 40, 20, 0, colWHITE, dwTempShader);

							if (bEntityName)
								SetTextCenteredWithBackGround(GetWeaponName(WeaponID), "fonts/smallFont", Location.x, Location.y, 0.4, 0.4, colWHITE, defaultShader);
						}
						else
							CG_DrawRotatedPicPhysical(0x827322C0, Location.x - 10, Location.y - 10, 20, 20, 0, colWHITE, Material_RegisterHandle("specialty_scavenger", 0));
