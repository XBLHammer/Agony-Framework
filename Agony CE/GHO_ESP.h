...
void _Radar()
{
	...
	RadarCenter.x = (Center.x * 2) - RadarSize - 7;
	RadarCenter.y = RadarSize / 2 + 50;
	...
	for (int i = 0; i < 2048; i++)
	{
		...
		if (Type == ET_MISSLE)
			...
			CG_DrawRotatedPicPhysical(0x82BCDD10, FinalX - 10, FinalY - 10, 20, 20, (i == cgArray_s->ClientNumber || Type == ET_ITEM) ? 0 : (cgArray_s->ViewAngle.y - Centity_s[i].viewAngle.y), color, Shader);
	}
}
...
void _Compass()
{
	...
	for (int i = 0; i < 2048; i++)
	{
		...
		if (Shader)
			CG_DrawRotatedPicPhysical(0x82BCDD10, CircleX - dwWidth / 2, CircleY - dwHeight / 2, dwWidth, dwHeight, FinalA, color, Shader);
	}
}
...
void _ESPClient(int client, float sColor[4])
{
		...
		WorldToScreen(HeadOrigin, &HeadLocation.x, &HeadLocation.y))
	{
		...
		if (bDrawWeapon) {
			...
			if (Shader) {
				CG_DrawRotatedPicPhysical(0x82BCDD10, FeetLocation.x - 30, FeetLocation.y, 60, 30, 0, colWHITE, Shader);
			}
		}

		if (bDrawName) {
			char *clientName = cgArray_s->Names[client].Name;
			char _dist[128];
				...
				if (Centity_s[i].Type == ET_ITEM)
				{
					if (Centity_s[i].Alive & 1) {
						int WeaponID = Centity_s[i].WeaponID;
							...
							int dwTempShader = GetWeaponShader(WeaponID);
							if (dwTempShader)
								CG_DrawRotatedPicPhysical(0x82BCDD10, Location.x - 20, Location.y, 40, 20, 0, colWHITE, dwTempShader);
