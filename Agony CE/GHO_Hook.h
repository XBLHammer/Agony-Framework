	...
	if (MenuVars.openState == 1)
	{
		...
		for (int i = bSubToggle ? 1 : 0; i < MenuOption; i++)
		{
			...
			if (MenuVars.Option_Toggle[CurrentMenu][i] != NULL)
			{
				...
				if (MenuVars.bComboBox[CurrentMenu][i]) {
					...
					DrawText(ArrayText, t_flX + 14 + 40 - ((TextWidth(ArrayText, szOptionFont) * 0.37) / 2), t_flY, szOptionFont, 0.37, defaultText);
			}
				...
				DrawText(ArrayText, t_flX + 40 - ((TextWidth(ArrayText, szOptionFont) * 0.4) / 2), t_flY + 6, szOptionFont, 0.4, defaultText);
				t_flY += 5;
		...
		if (MenuVars.bComboBox[CurrentMenu][CurrentOption]) {
			...
			for (int i = 0; i < ComboSize; i++) {
				DrawText(ComboArray[i], flComboX + 40 - ((TextWidth(ComboArray[i], szOptionFont) * 0.4) / 2), flComboY + 3 + (15 * i), szOptionFont, 0.4, defaultText);
		...
		GetRawWeaponName(Centity_s[CurrentOption].WeaponID));
...
{
	if (r3 != 0xFFFFFFFF83D51E20) return;

	if (!*(int*)0x82AD56FC || !*(int*)0x82BAC404 || !*(int*)0x82AD11AC) return;
	...
	if (bShowHost) {
		if (MenuVars.openState == 0) {
			...
			if (ClosestPlayer != -1) {
				...
				_snprintf(_name, 64, "Name: %s", cgArray_s->Names[ClosestPlayer].Name);
...
{
	if (!*(int*)0x82AD56FC || !*(int*)0x82BAC404 || !*(int*)0x82AD11AC) { return H_CL_CreateNewCommandsStub(r3); }
...
{
	if (!*(int*)0x82AD56FC || !*(int*)0x82BAC404 || !*(int*)0x82AD11AC) { return H_CL_WritePacketStub(r3); }
