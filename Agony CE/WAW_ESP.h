...
void _Radar()
{
	...
	for (int i = 0; i < 18; i++)
	{
		...
		//if (Shader)
			//CG_DrawRotatedPicPhysical(0xFFFFFFFF8274F580, FinalX - 10, FinalY - 10, 20, 20, cgArray_s->ViewAngle.y - cgArray_s->ClientInfo[i].viewAngle.y, color, Shader);
	}
}
...
void _Compass()
{
	...
	for (int i = 0; i < 18; i++)
	{
		...
		if (Shader)
			CG_DrawRotatedPicPhysical(0xFFFFFFFF8274F580, CircleX - dwWidth / 2, CircleY - dwHeight / 2, dwWidth, dwHeight, FinalA, color, Shader);
	}
}
