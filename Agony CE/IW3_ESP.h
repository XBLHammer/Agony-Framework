...
void _Radar()
{
	...
	for (int i = 0; i < 18; i++)
	{
		...
		if (Shader)
			CG_DrawRotatedPicPhysical(0xFFFFFFFF8246F308, FinalX - 10, FinalY - 10, 20, 20, cgArray_s->ViewAngle.y - cgArray_s->ClientInfo[i].viewAngle.y, color, Shader);
	}
}
...
void _Compass()
{
	...
	for (int i = 0; i < 18; i++)
	{
		...
		if (Shader)
			CG_DrawRotatedPicPhysical(0xFFFFFFFF8246F308, CircleX - dwWidth / 2, CircleY - dwHeight / 2, dwWidth, dwHeight, FinalA, color, Shader);
	}
}
...
void DrawBoneLine(pCentity_t entity, int ent, const char* tagname1, const char* tagname2, float *col)
{
	...
	if (WorldToScreen(Origin1, &Loc1.x, &Loc1.y) && WorldToScreen(Origin2, &Loc2.x, &Loc2.y)) {
		DrawLine(Loc1.x, Loc1.y, Loc2.x, Loc2.y, col, Material_RegisterHandle("white", 0), 2.25);
	}
}
